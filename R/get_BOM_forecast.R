...
stations <-
  readr::read_table(paste0(tempdir(), "stations.zip"),
                    skip = 2,
                    col_names = TRUE)[-1,]
...
xmlfile <-
  XML::xmlTreeParse("ftp://ftp.bom.gov.au/anon/gen/fwo/IDQ11295.xml")
topxml <- XML::xmlRoot(xmlfile)

topxml1 <-
  XML::xmlSApply(topxml, function(x)
    XML::xmlSApply(x, XML::xmlAttrs))
...
# column later
length_districts <-
  as.numeric(abs(diff(which(
    bits == "public-district"
  )))) - 1
...
bits <-
  as.data.frame(cbind(rep(unlist(state), length(bits[-c(1:3)])),
                      unlist(districts),
                      unlist(bits[-c(1:3)])))
...
bits$location <- toupper(bits$location)
...
Australian_Post_Codes_Lat_Lon <-
  cbind(Australian_Post_Codes_Lat_Lon,
        ...
        location_lat_lon$lon)
Australian_Post_Codes_Lat_Lon <-
  rename(
    Australian_Post_Codes_Lat_Lon,
    c(
      "location_lat_lon$lat" = "lat",
      "location_lat_lon$lon" = "lon"
    )
  )
...
col_names <-
  c(
    ...
    paste("minT", day_1),
    paste("maxT", day_1),
    paste("description", day_1),
    paste("chance_of_rain", day_1),
    paste("minR", day_2),
    paste("maxR", day_2),
    paste("minT", day_2),
    paste("maxT", day_2),
    paste("description", day_2),
    paste("chance_of_rain", day_2),
    paste("minR", day_3),
    paste("maxR", day_3),
    paste("minT", day_3),
    paste("maxT", day_3),
    paste("description", day_3),
    paste("chance_of_rain", day_3),
    paste("minR", day_4),
    paste("maxR", day_4),
    paste("minT", day_4),
    paste("maxT", day_4),
    paste("description", day_4),
    paste("chance_of_rain", day_4),
    paste("minR", day_5),
    paste("maxR", day_5),
    paste("minT", day_5),
    paste("maxT", day_5),
    paste("description", day_5),
    paste("chance_of_rain", day_5),
    paste("minR", day_6),
    paste("maxR", day_6),
    paste("minT", day_6),
    paste("maxT", day_6),
    paste("description", day_6),
    paste("chance_of_rain", day_6)
  )
...
for (m in master_run) {
  ...
  for (x in run) {
    ...
    bit <-
      paste(substr(bit, 1, 4), "sub", substr(bit, 5, nchar(bit)), sep = "")
    ...
    day_df <-
      data.frame(minR, maxR, minT, maxT, description, chance_of_rain)
  ...
  week_df <-
    cbind(day_df_1, day_df_2, day_df_3, day_df_4, day_df_5, day_df_6)
