...
#' \code{\link{make_hourly_forecast}} function and returns a list of hourly
#' surfaces for the next six-days forecast at 250x250m resolution. List names
#' are the Julian day followed by the hour, e.g., "316.1" being the 316th day
#' of the year and the first hour of the 24 hours.
#'
...
interpolate_hourly <- function(downscaled_temp) {
  ...
  tps_pred <- lapply(spline_list, function(y)
   raster::interpolate(model = y, object = Oz_dem, xyOnly = FALSE))
